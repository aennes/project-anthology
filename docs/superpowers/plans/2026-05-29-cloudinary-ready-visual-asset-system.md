# Cloudinary-Ready Visual Asset System — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a deterministic, idempotent asset pipeline (discover→rank→stage→approve→plan-upload→verify) with a canonical manifest, zero actual Cloudinary uploads, and a runtime helper for the React app.

**Architecture:** Single CLI entrypoint at `scripts/asset-pipeline.ts` that fetches entities from Jolpica API, searches Wikimedia Commons for open-licensed image candidates, scores/ranks them, downloads to local staging, and writes a canonical JSON manifest. A thin `utils/assetManifest.ts` reads the approved manifest at runtime with graceful fallbacks.

**Tech Stack:** TypeScript + tsx, Node.js built-ins (fs/crypto/path), Vitest for tests, Vite build chain untouched.
...
| File | Action | Purpose |
|------|--------|---------|
| `scripts/asset-pipeline.ts` | CREATE | CLI entrypoint + all pipeline logic, fully exported |
| `utils/assetManifest.ts` | CREATE | Runtime manifest reader, browser-safe |
| `scripts/data/asset-manifest.json` | CREATE | Canonical manifest (empty initial) |
| `public/data/asset-manifest.json` | CREATE | Runtime copy (empty initial) |
| `tests/asset-pipeline/ranking.test.ts` | CREATE | Score function tests |
| `tests/asset-pipeline/id-mapping.test.ts` | CREATE | ID normalization tests |
| `tests/asset-pipeline/manifest-schema.test.ts` | CREATE | validateManifest tests |
| `tests/asset-pipeline/verify.test.ts` | CREATE | runVerify tests |
| `tests/asset-pipeline/fallback.test.ts` | CREATE | assetManifest fallback tests |
| `package.json` | MODIFY | Add 6 pipeline:* scripts |
| `utils/assetPipeline.ts` | UNTOUCHED | Legacy Cloudinary fallback |
| `utils/imageCDN.ts` | UNTOUCHED | Legacy CDN helper |

---

## Task 1: Failing tests — scoring (ranking.test.ts)

**Files:**
- Create: `tests/asset-pipeline/ranking.test.ts`

- [ ] **Step 1.1: Create the failing test file**

```typescript
// tests/asset-pipeline/ranking.test.ts
...
describe('scoreCandidate determinism', () => {
  const c = { url: 'https://upload.wikimedia.org/a.jpg', license: 'CC BY-SA 4.0', width: 1800, height: 1013, author: 'A', source_page: '' };
...
Run: `npx vitest run tests/asset-pipeline/ranking.test.ts`
Expected: FAIL with "Cannot find module '../../scripts/asset-pipeline'"

---

## Task 2: Create asset-pipeline.ts — types + scoring (Task 1 → PASS)

**Files:**
- Create: `scripts/asset-pipeline.ts`

- [ ] **Step 2.1: Create the file with types, constants, and scoring exports**
 ...
 *
 * All scoring/validation functions are exported for testing.
 * NO Cloudinary uploads are performed — plan-upload command produces a plan only.
 */
import fs from 'fs';
...
// Radio: editorial content — no standard API
...
export function validateManifest(manifest: unknown): ValidationResult {
  ...
  for (let i = 0; i < (m['assets'] as unknown[]).length; i++) {
    const e = (m['assets'] as Record<string, unknown>[])[i];
    const p = `assets[${i}]`;
    if (!VALID_TYPES.has(String(e['asset_type'] ?? ''))) errors.push(`${p}.asset_type invalid: "${e['asset_type']}"`);
    if (!validateEntityId(String(e['entity_id'] ?? ''))) errors.push(`${p}.entity_id invalid: "${e['entity_id']}"`);
    if (!VALID_STATUSES.has(String(e['status'] ?? ''))) errors.push(`${p}.status invalid: "${e['status']}"`);
    const score = Number(e['candidate_score']);
    if (!isNaN(score) && (score < 0 || score > 100)) errors.push(`${p}.candidate_score out of range: ${score}`);
  }
...
async function fetchWithRetry(url: string, attempts = 4): Promise<Response> {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    console.warn(`[rate-limit] retrying ${url} in ${delay}ms`);
    await sleep(delay);
...
export async function fetchJolpicaDrivers(year: number): Promise<Array<{ code: string; name: string }>> {
  ...
  const standings = (data?.['MRData'] as Record<string, unknown>)?.['StandingsTable'] as Record<string, unknown> | undefined;
  const list = (standings?.['StandingsLists'] as unknown[])?.[0] as Record<string, unknown> | undefined;
  const rows = (list?.['DriverStandings'] as Record<string, unknown>[]) ?? [];
  return rows.flatMap((s) => {
    const d = s['Driver'] as Record<string, string> | undefined;
    const code = normalizeEntityId(String(d?.['code'] ?? ''));
    if (!code) return [];
    return [{ code, name: `${d?.['givenName'] ?? ''} ${d?.['familyName'] ?? ''}`.trim() || code }];
  });
}
...
export async function fetchJolpicaTeams(year: number): Promise<Array<{ constructorId: string; name: string }>> {
  ...
  const standings = (data?.['MRData'] as Record<string, unknown>)?.['StandingsTable'] as Record<string, unknown> | undefined;
  const list = (standings?.['StandingsLists'] as unknown[])?.[0] as Record<string, unknown> | undefined;
  const rows = (list?.['ConstructorStandings'] as Record<string, unknown>[]) ?? [];
  return rows.flatMap((s) => {
...
export async function fetchJolpicaCircuits(year: number): Promise<Array<{ circuitId: string; name: string; country: string }>> {
  ...
  const table = (data?.['MRData'] as Record<string, unknown>)?.['CircuitTable'] as Record<string, unknown> | undefined;
...
export async function searchWikimediaCandidates(searchTerm: string, type: AssetType): Promise<RawCandidate[]> {
  ...
  for (const p of pages) {
    ...
    const source_page = fileTitle ? `https://commons.wikimedia.org/wiki/File:${encodeURIComponent(fileTitle)}` : '';
...
function buildEntry(type: AssetType, id: string, name: string, candidates: ScoredCandidate[]): AssetEntry {
  return {
    ...
    candidates, candidate_score: 0, local_staged_path: null, checksum_sha256: null,
    staged_at: null, planned_cloudinary_public_id: null,
    planned_cloudinary_folder: CLOUDINARY_FOLDER,
    approved: false, approved_at: null, approved_by: null,
    status: 'discovered', discovered_at: new Date().toISOString(),
    ranked_at: null, uploaded_at: null, error: null,
  };
}
...
async function cmdRank(opts: RankOpts): Promise<void> {
  ...
  for (const entry of manifest.assets) {
    ...
    if (!entry.candidates.length) { console.warn(`[rank] no candidates: ${entry.asset_type}/${entry.entity_id}`); continue; }
...
async function cmdStage(opts: StageOpts): Promise<void> {
  ...
  for (const entry of manifest.assets) {
    ...
    if (!entry.source_url) { console.warn(`[stage] no source_url: ${entry.asset_type}/${entry.entity_id}`); continue; }
    const dest = stagingPath(entry.asset_type, entry.entity_id);
    if (!opts.force && fs.existsSync(dest) && fs.statSync(dest).size > 512) {
      const existing = computeFileHash(dest);
      if (existing === entry.checksum_sha256) {
...
async function cmdPlanUpload(opts: { dryRun: boolean }): Promise<void> {
  ...
  const plan = {
    ...
    note: 'Upload is DISABLED. Set CLOUDINARY_URL and add upload command to activate.',
...
- [ ] **Step 2.2: Run ranking tests — verify PASS**
...
```bash
git add scripts/asset-pipeline.ts tests/asset-pipeline/ranking.test.ts
git commit -m "feat: add asset pipeline core with scoring functions"
```

---

## Task 3: Failing tests — ID mapping + manifest schema

**Files:**
- Create: `tests/asset-pipeline/id-mapping.test.ts`
- Create: `tests/asset-pipeline/manifest-schema.test.ts`

- [ ] **Step 3.1: Create id-mapping.test.ts**

```typescript
// tests/asset-pipeline/id-mapping.test.ts
// @vitest-environment node
import { describe, it, expect } from 'vitest';
...
- [ ] **Step 3.2: Create manifest-schema.test.ts**

```typescript
// tests/asset-pipeline/manifest-schema.test.ts
// @vitest-environment node
import { describe, it, expect } from 'vitest';
import { validateManifest } from '../../scripts/asset-pipeline';

const VALID_ENTRY = {
  asset_type: 'driver', entity_id: 'ver', display_name: 'Max Verstappen',
  source_url: null, source_page: null, license: null, attribution: null,
  candidates: [], candidate_score: 0, local_staged_path: null, checksum_sha256: null,
  staged_at: null, planned_cloudinary_public_id: null,
  planned_cloudinary_folder: 'f1-anthology', approved: false, approved_at: null,
  approved_by: null, status: 'discovered', discovered_at: '2026-05-29T10:00:00Z',
  ranked_at: null, uploaded_at: null, error: null,
};

const VALID_MANIFEST = {
  version: 2, generated_at: '2026-05-29T10:00:00Z', assets: [VALID_ENTRY],
};
...
describe('validateManifest', () => {
  ...
  it('rejects missing version', () => {
    const m = { ...VALID_MANIFEST }; delete (m as Record<string, unknown>)['version'];
  ...
  it('rejects invalid status', () => {
    expect(validateManifest({ ...VALID_MANIFEST, assets: [{ ...VALID_ENTRY, status: 'invalid' }] }).valid).toBe(false);
  });
  it('rejects invalid asset_type', () => {
    expect(validateManifest({ ...VALID_MANIFEST, assets: [{ ...VALID_ENTRY, asset_type: 'unknown' }] }).valid).toBe(false);
  });
  it('rejects path traversal in entity_id', () => {
    expect(validateManifest({ ...VALID_MANIFEST, assets: [{ ...VALID_ENTRY, entity_id: '../passwd' }] }).valid).toBe(false);
  });
  it('rejects score > 100', () => {
    expect(validateManifest({ ...VALID_MANIFEST, assets: [{ ...VALID_ENTRY, candidate_score: 150 }] }).valid).toBe(false);
  });
  it('rejects score < 0', () => {
    expect(validateManifest({ ...VALID_MANIFEST, assets: [{ ...VALID_ENTRY, candidate_score: -5 }] }).valid).toBe(false);
  });
});
```

- [ ] **Step 3.3: Run — verify PASS (functions already exist in asset-pipeline.ts)**

Run: `npx vitest run tests/asset-pipeline/id-mapping.test.ts tests/asset-pipeline/manifest-schema.test.ts`
Expected: All tests PASS

- [ ] **Step 3.4: Commit**

```bash
git add tests/asset-pipeline/id-mapping.test.ts tests/asset-pipeline/manifest-schema.test.ts
git commit -m "test: add ID mapping and manifest schema tests"
```

---

## Task 4: Failing tests + impl — verify (verify.test.ts)

**Files:**
- Create: `tests/asset-pipeline/verify.test.ts`

- [ ] **Step 4.1: Create verify.test.ts**

```typescript
// tests/asset-pipeline/verify.test.ts
// @vitest-environment node
import { describe, it, expect } from 'vitest';
...
const BASE_ENTRY = {
  asset_type: 'circuit' as const, entity_id: 'monaco',
  ...
  license: 'CC BY-SA 4.0', attribution: 'Test Author / Wikimedia Commons',
  candidates: [], candidate_score: 72, local_staged_path: null,
  checksum_sha256: null, staged_at: null,
  planned_cloudinary_public_id: 'f1-anthology/circuit/monaco',
  planned_cloudinary_folder: 'f1-anthology', approved: false,
  approved_at: null, approved_by: null, status: 'ranked' as const,
  discovered_at: '2026-05-29T10:00:00Z', ranked_at: '2026-05-29T10:01:00Z',
  uploaded_at: null, error: null,
};

const wrap = (overrides = {}): AssetManifest => ({
  version: 2, generated_at: '2026-05-29T10:00:00Z',
  assets: [{ ...BASE_ENTRY, ...overrides }],
});
...
describe('runVerify', () => {
  ...
  it('passes an empty manifest', () => {
    expect(runVerify({ version: 2, generated_at: '2026-05-29T10:00:00Z', assets: [] }).passed).toBe(true);
  });
  it('fails when license empty for staged entry', () => {
    const r = runVerify(wrap({ status: 'staged', license: '', local_staged_path: '/p', checksum_sha256: 'abc' }));
    expect(r.passed).toBe(false);
    expect(r.errors.some((e) => e.includes('license'))).toBe(true);
  });
  it('fails when checksum missing for staged entry', () => {
    const r = runVerify(wrap({ status: 'staged', license: 'CC BY-SA 4.0', local_staged_path: '/p', checksum_sha256: null }));
    expect(r.passed).toBe(false);
    expect(r.errors.some((e) => e.includes('checksum'))).toBe(true);
  });
  it('fails when local_staged_path missing for staged entry', () => {
    const r = runVerify(wrap({ status: 'staged', license: 'CC BY-SA 4.0', local_staged_path: null, checksum_sha256: 'abc' }));
    expect(r.passed).toBe(false);
  ...
  it('fails when approved entry lacks planned_cloudinary_public_id', () => {
    const r = runVerify(wrap({
      status: 'approved', approved: true, local_staged_path: '/p',
      checksum_sha256: 'abc', license: 'CC BY-SA 4.0',
...
Run: `npx vitest run tests/asset-pipeline/verify.test.ts`
Expected: All tests PASS

- [ ] **Step 4.3: Commit**

```bash
git add tests/asset-pipeline/verify.test.ts
git commit -m "test: add runVerify tests"
```

---

## Task 5: Failing tests + impl — runtime manifest fallback

**Files:**
- Create: `tests/asset-pipeline/fallback.test.ts`
- Create: `utils/assetManifest.ts`

- [ ] **Step 5.1: Create failing fallback.test.ts**

```typescript
// tests/asset-pipeline/fallback.test.ts
import { describe, it, expect, beforeEach } from 'vitest';
import { getAssetUrlFromManifest, PLACEHOLDERS, injectManifestForTesting } from '../../utils/assetManifest';
import type { AssetManifest } from '../../scripts/asset-pipeline';
...
const makeManifest = (overrides: Record<string, unknown> = {}): AssetManifest => ({
  ...
  assets: [{
    ...
    source_page: null, license: 'CC BY-SA 4.0', attribution: null,
    candidates: [], candidate_score: 80,
    local_staged_path: 'scripts/data/staging/driver/ver.webp',
    checksum_sha256: 'abc', staged_at: '2026-05-29T10:05:00Z',
    planned_cloudinary_public_id: 'f1-anthology/driver/ver',
    planned_cloudinary_folder: 'f1-anthology',
    approved: true, approved_at: '2026-05-29T11:00:00Z', approved_by: 'cli',
    status: 'approved', discovered_at: '2026-05-29T10:00:00Z',
    ranked_at: '2026-05-29T10:01:00Z', uploaded_at: null, error: null,
    ...overrides,
  }],
});
...
describe('getAssetUrlFromManifest', () => {
  it('returns source_url for approved entry', () => {
    const url = getAssetUrlFromManifest(makeManifest(), 'driver', 'ver');
    expect(url).toBe('https://upload.wikimedia.org/ver.jpg');
  });
  ...
  it('returns placeholder when entry not approved', () => {
    expect(getAssetUrlFromManifest(makeManifest({ approved: false }), 'driver', 'ver')).toBe(PLACEHOLDERS.driver);
  });
  ...
  it('returns placeholder when source_url is null', () => {
    expect(getAssetUrlFromManifest(makeManifest({ source_url: null }), 'driver', 'ver')).toBe(PLACEHOLDERS.driver);
  });
  it('returns correct placeholder for each type', () => {
    expect(PLACEHOLDERS.driver).toContain('/images/placeholders/driver');
    expect(PLACEHOLDERS.team).toContain('/images/placeholders/team');
    expect(PLACEHOLDERS.circuit).toContain('/images/placeholders/circuit');
    expect(PLACEHOLDERS.radio).toContain('/images/placeholders/radio');
  });
});

describe('injectManifestForTesting', () => {
  it('allows injecting manifest for sync tests', () => {
    injectManifestForTesting(makeManifest());
    // The injection is used by getAssetUrl (cached-manifest path)
    // Direct function test via getAssetUrlFromManifest is sufficient
...
Run: `npx vitest run tests/asset-pipeline/fallback.test.ts`
Expected: FAIL with "Cannot find module '../../utils/assetManifest'"

- [ ] **Step 5.3: Create utils/assetManifest.ts**
 ...
 *
 * Priority: approved manifest entry → placeholder SVG
 * The legacy utils/assetPipeline.ts is NOT modified.
 */
...
/** Load the runtime manifest once from /data/asset-manifest.json. */
export async function loadRuntimeManifest(): Promise<void> {
  ...
  try {
    ...
    // Graceful: manifest unavailable, fallbacks active
  }
}

/**
 * Pure lookup against a given manifest — does not use module-level cache.
 * Falls back to placeholder when manifest null, entry missing, not approved, or no URL.
 */
...
/** Get attribution for a specific asset entry from the cached manifest. */
...
/** True once loadRuntimeManifest() has been called and manifest loaded successfully. */
export function isManifestReady(): boolean {
  return _loadAttempted && _manifest !== null;
}
```

- [ ] **Step 5.4: Run fallback tests — verify PASS**
...
```bash
git add utils/assetManifest.ts tests/asset-pipeline/fallback.test.ts
git commit -m "feat: add runtime manifest helper with fallback chain"
```

---

## Task 6: Initial manifest files + package.json scripts

**Files:**
- Create: `scripts/data/asset-manifest.json`
- Create: `public/data/asset-manifest.json`
- Modify: `package.json`
- Create: `scripts/data/.gitignore`

- [ ] **Step 6.1: Create empty canonical manifest**
...
```
Save to: `scripts/data/asset-manifest.json`

- [ ] **Step 6.2: Create empty runtime manifest**

Same content, save to: `public/data/asset-manifest.json`

- [ ] **Step 6.3: Create staging gitignore**

```
staging/
upload-plan.json
```
Save to: `scripts/data/.gitignore`

- [ ] **Step 6.4: Add pipeline scripts to package.json**

Add these 6 entries to the `"scripts"` section:
```json
"pipeline:discover": "tsx scripts/asset-pipeline.ts discover",
"pipeline:rank":     "tsx scripts/asset-pipeline.ts rank",
"pipeline:stage":    "tsx scripts/asset-pipeline.ts stage",
"pipeline:approve":  "tsx scripts/asset-pipeline.ts approve",
"pipeline:plan":     "tsx scripts/asset-pipeline.ts plan-upload",
"pipeline:verify":   "tsx scripts/asset-pipeline.ts verify"
```

- [ ] **Step 6.5: Commit**

```bash
git add scripts/data/asset-manifest.json public/data/asset-manifest.json scripts/data/.gitignore package.json
git commit -m "feat: add initial manifest files and pipeline npm scripts"
```
...
- [ ] **Step 7.1: Run all pipeline tests**

Run: `npx vitest run tests/asset-pipeline/`
Expected: All tests in 5 files PASS
...
Run: `npm run build`
Expected: Build completes without errors. The `utils/assetManifest.ts` `import type` does not pull Node modules into the browser bundle.

- [ ] **Step 7.4: Fix any TypeScript errors**

If `noUnusedLocals` or `noUnusedParameters` errors appear, prefix unused params with `_`.

- [ ] **Step 7.5: Final commit**

```bash
git add -A
git commit -m "feat: Cloudinary-ready Visual Asset System — pipeline + tests complete"
```
